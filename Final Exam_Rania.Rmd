...
I will run a regression model that will predict the effect on the chance in distance to transit on the percent change in sales. I will control for change in parking supply + changes in parking price + changes in bike traffic. 
...
For every one-mile increase in the distance from a transit stop, sales (decrease) by (17-percentage) points, controlling for difference in the changes to parking costs, parking supply and bike volumes. That is also statistically significant.


## Question 4

Was the effect of transit on sales different for some types of businesses than for others? What 
was the difference, if any? 

I will estimate a regression model with an interaction between transit distance and business type. For cosistence with the previous model, I will keep the controls of parking and cycling. I am hypothesizing that distance from a transit stop gas an effect on different types of store when considering how much thier sales have changed. 
...
For gas stations, furniture stores, and grocery stores, the distance to transit did not have a substantial effect on the change in sales. For clothing stores, bookstores, drugstores, and restaurants, each additional mile closer to transit was associated with an additional (30-percentage increase) in sales or in other words, one mile each one mile further the store is from transit it loses aprx 30-percentage of it sales.
